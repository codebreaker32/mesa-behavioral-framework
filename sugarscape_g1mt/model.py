...
class SugarscapeG1mt(mesa.Model):
    ...
    def __init__(self, scenario: SugarScapeScenario = SugarScapeScenario):
        ...
        self.data_registry.track_model(
            self, "model_data", ["trader_count", "trade_volume", "price"]
    ...
    @property
    def trader_count(self):
        return len(self.agents)
    
    @property
    def trade_volume(self):
        return sum(len(a.trade_partners) for a in self.agents)
    
    @property
    def price(self):
        return geometric_mean(flatten([a.prices for a in self.agents]))
...
if __name__ == "__main__":
    for rng in [42,99,1234,1467]:
        model = SugarscapeG1mt(scenario=SugarScapeScenario(rng=rng))
        model.run_for(500)
        df = model.recorder.get_table_dataframe("model_data")
        df.to_csv(f"data/sugarscape_rng{rng}.csv", index=False)